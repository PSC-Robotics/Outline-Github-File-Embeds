...
class LightboxGithubEmbedImage extends LightboxImage {
  constructor(view: EditorView, pos: number) {
    super();
    this.pos = pos;
    const node = view.state.doc.nodeAt(pos);
    const href = node?.attrs.href ?? "";
    this.src = `/api/urls.githubSvg?url=${encodeURIComponent(href)}`;
    this.alt = "GitHub File";
    this.source = "github";
    this.element = view.nodeDOM(pos) as HTMLDivElement;
  }
...
const isGithubEmbed = (node: Node) =>
  node.type.name === "embed" && node.attrs.href?.startsWith("https://github.com/");
...
export class LightboxImageFactory {
  static createLightboxImage(view: EditorView, pos: number): LightboxImage {
    ...
    if (isGithubEmbed(node)) {
      return new LightboxGithubEmbedImage(view, pos);
    }
...
export const isLightboxNode = (node: Node) =>
  isImage(node) || isMermaid(node) || isGithubEmbed(node);
