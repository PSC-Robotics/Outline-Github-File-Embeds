...
router.get(
  ...
  async (ctx: any) => {
    ...
    try {
      ...
      // Add a timestamp parameter to bust GitHub's Fastly CDN cache
      const proxyUrl = `http://10.10.10.104:3456/content?url=${encodeURIComponent(url)}&t=${Date.now()}`;
      ...
      ctx.set("Content-Type", response.headers.get("content-type") || "image/svg+xml");
